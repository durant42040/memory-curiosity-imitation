...
if __name__ == "__main__":
    ...
    for i in range(args.procs):
        # envs.append(FullyObsWrapper(utils.make_env(args.env, args.seed + 10000 * i)))
        env = utils.make_env(args.env, args.seed + 10000 * i)
        # print(f"before env: {env}")
        # print(f"before image shape: {env.observation_space.spaces}")
        if args.full_obs:
            env = FullyObsWrapper(env)
        # print(f"after env: {env}")
        # print(f"after image shape: {env.observation_space.spaces}")
        envs.append(env)
