...
class ValueIteration():
    def __init__(self, env, discount_factor: float = 1.0, robust=False):
        self.env = env
        ...
        while True:
            ...
            for state in range(env_cloned.get_state_space()):
                ...
                for action in range(env_cloned.get_action_space()):
                    ...
                    if self.robust:
                        _, reward, terminated, truncated, _ = env_cloned.robust_step(action)
                    else:
                        _, reward, terminated, truncated, _ = env_cloned.step(action)
                    next_state = env_cloned.agent_information_to_state()
            ...
            if delta < self.threshold:
                break
        # if self.robust:
        #     print(self.env.state_rewards.sum())
        #     for i in range(self.env.height):
        #         print(self.env.state_rewards[i*self.env.width:(i+1)*self.env.width])
        #     # for i in range(self.env.get_state_space()):
    ...
    def policy_improvement(self):
        ...
        for state in range(env_cloned.get_state_space()):
            ...
            for action in range(env_cloned.get_action_space()):
                ...
                env_cloned.state_to_agent_information(state)
                if self.robust:
                    prev_state = env_cloned.agent_information_to_state()
                    _, reward, terminated, truncated, _ = env_cloned.robust_step(action)
                    # if (reward != 0):
                    #     print(reward, action, prev_state)
                else:
                    _, reward, terminated, truncated, _ = env_cloned.step(action)
                next_state = env_cloned.agent_information_to_state()
                end = terminated or truncated
        ...
        # if self.robust:
        #     print(self.env.action_rewards.sum())
        #     for i in range(self.env.height):
        #         print(self.policy[i*self.env.width:(i+1)*self.env.width])
        #     print()
