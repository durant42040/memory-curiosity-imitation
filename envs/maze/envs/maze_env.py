...
from envs.maze.envs.maze_view_2d import MazeView2D
...
class MazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.partial_obs:
            self.state = self.get_surrounding_walls()
        else:
    ...
    def get_surrounding_walls(self):
        position = tuple(self.maze_view.robot)
        return [
            self.maze_view.maze.is_open(position, dir)
            for dir in self.maze_view.maze.COMPASS
        ]
