...
sys.path.append('/workspace/DIFO-on-POMDP/skill_based/')
...
class TileRewardNet(RewardNet):
    def __init__(
        ...
        **kwargs,
        ...
        # Embedding layers for the observation space
        ...
        # self.state_embedding = nn.Embedding(num_states, self.state_dim)
        self.action_embedding = nn.Linear(num_actions, self.action_dim)
        ...
        n = self.num_views
        m = self.num_views
        cnn_out_size = ((n - 1) // 2 - 2) * ((m - 1) // 2 - 2) * 64
        mlp_input_size = cnn_out_size
        if self.use_action:
            mlp_input_size += 7 # Action embedding size
        if self.use_done:
            mlp_input_size += 1

        self.fc1 = nn.Linear(mlp_input_size, 128)
        self.fc2 = nn.Linear(128, 1)

    def forward(self, state, action, next_state, done):
        if self.use_state:
            # Split the input into the three components
            object_idx = state[:, 0, :, :].long()
            color_idx = state[:, 1, :, :].long()
            # state_idx = state[:, 2, :, :].long()

            # Apply embeddings
            object_emb = self.object_embedding(object_idx)  # Shape: [batch, H, W, object_dim]
            color_emb = self.color_embedding(color_idx)     # Shape: [batch, H, W, color_dim]
            # state_emb = self.state_embedding(state_idx)     # Shape: [batch, H, W, state_dim]

            # Permute embeddings to [batch, dim, H, W] format for conv layers
            # print(object_emb.shape)
            object_emb = object_emb.permute(0, 3, 1, 2)
            color_emb = color_emb.permute(0, 3, 1, 2)
            # state_emb = state_emb.permute(0, 3, 1, 2)

            # Concatenate embeddings along the channel dimension
            x = th.cat((object_emb, color_emb), dim=1)
            # print("xshape", x.shape)  # Total channels: 16

            # Pass through convolutional layers
            x = F.relu(self.image_conv(x))
            # print("xshape", x.shape)  # Total channels: 128

            # Make tensor contiguous and flatten the output
            x = x.contiguous().view(x.size(0), -1)
            # print("xshape", x.shape) 

        if self.use_action:
            # Convert action to Long tensor before embedding
            # action = self.action_embedding(action)
            x = th.cat((x, action), dim=1)

        if self.use_done:
            x = th.cat((x, done.float()), dim=1)


        # Pass through fully connected layers
        x = F.relu(self.fc1(x)).squeeze(-1)
        outputs = self.fc2(x).squeeze(-1)

        return outputs

class DiscRewardNet(RewardNet):
    def __init__(
        self,
        observation_space: gym.Space,
        action_space: gym.Space,
    ...
    def forward(self, state, action, next_state, done):
        outputs = self.discriminator(state)
